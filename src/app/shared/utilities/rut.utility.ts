 ...
 * @param {string} rut - rut entregado
 */
export function isFormatRutValid(rut: string): boolean {
  let _rut = getCleanRut(rut);
...
export function isRutValid(rut: string): boolean {
  return isFormatRutValid(rut) && isDvValid(rut);
}
 ...
 * @param {string} rut - rut entregado
 */
export function isDvValid(rut: string): boolean {
 ...
 * @param {string} rut - rut entregado
 */
export function getRutFormat(rut: string) {
  if (rut.length === 1 || !isFormatRutValid(rut)) {
    return rut;
  }
...
export function getRutStatus(rut_con_dv: string) {
  return {
    ...
    formato: getRutFormat(rut_con_dv),

    isRutValido: isRutValid(rut_con_dv),
    isDvValido: isDvValid(rut_con_dv),
    isFormatoValido: isFormatRutValid(rut_con_dv),
  };
}
